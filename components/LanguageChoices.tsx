...
const LanguageChoices: React.FC<LanguageChoicesProps> = ({ 
  ...
  const showWordTranslation = (event: React.MouseEvent, translation: string) => {
    ...
    <div>
      ...
      {options.map((option, index) => (
        <div
          ...
          className={`bg-white/5 border-2 border-white/10 rounded-2xl p-4 mb-3 cursor-pointer transition-all duration-300 hover:bg-indigo-600/20 hover:border-indigo-400 ${
          ...
          <div className="text-white text-lg mb-1">
            {option.words.map((word, wordIndex) => (
              <span
                ...
                onClick={(e) => showWordTranslation(e, word.translation)}
              >
                {word.text}
              </span>
            ))}
      ...
      {tooltipData.show && (
        <div 
          className="fixed bg-indigo-600 text-white px-3 py-2 rounded-lg text-sm z-50 shadow-lg transition-opacity duration-300"
          style={{ 
            left: tooltipData.x, 
            top: tooltipData.y,
